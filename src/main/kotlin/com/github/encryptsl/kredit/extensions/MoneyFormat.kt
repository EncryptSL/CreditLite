...
private fun getLocale(localeStr: String): Locale {
    ...
    return when (parts.size) {
        1 -> Locale.Builder().setLocale(Locale.of(parts[0])).build()
        2 -> Locale.Builder().setLocale(Locale.of(parts[0], parts[1])).build()
        else -> Locale.Builder().setLocale(Locale.of(parts[0], parts[1], parts[2])).build()
    }
}